...
public class ContentProviderReconcileApp extends JPanel implements ActionListener, PropertyChangeListener {
  ...
  public static void createAndShowGUI(OIObjectManager paramOIObjectManager, HashSet<String> paramHashSet) {
    ...
    ContentProviderReconcileApp contentProviderReconcileApp = new ContentProviderReconcileApp(jDialog, paramOIObjectManager, paramHashSet);
  }
  ...
  private boolean doLoad() {
    ...
    try {
      this.tm.loadUnReconciledProperties(this.om, this.mpnIDSet, this.errorHash);
    ...
    if (this.tm.getMPNCount() == 0)
      i = JOptionPane.showConfirmDialog(this.frame, "No unreconciled Manufacturer Parts found.  Exit Reconcile application?", "EDM Library/External Content Property Reconcile", 0); 
    if (i == 0)
      this.frame.dispose(); 
    return (i != 0);
  }
  ...
  private void doReconcile() {
    ...
    if (bool2) {
      int j = JOptionPane.showConfirmDialog(this, "You have indicated an Action of 'Ignore Always' on one or more properties.\nThese property(s) will be ignored until the Reconcile Action's 'Ignore' flag is reset in the Manufacturer Part's External Content object.\n\nContinue?");
      if (j != 0)
        return; 
    } 
  ...
  public void propertyChange(PropertyChangeEvent paramPropertyChangeEvent) {
    if ("progress" == paramPropertyChangeEvent.getPropertyName() && (this.progressMonitor.isCanceled() || this.task.isDone())) {
...
/* Location:              C:\Users\z004ut2y\OneDrive - Siemens AG\Documents\01_Projects\Customers\Var Industries\varindustries_edm-eles-sample-dataset_2025-09-18_1349 (1)\Part Aggeration Service\ContentProviderCore.jar!\com\mentor\dms\contentprovider\sync\gui\ContentProviderReconcileApp.class
