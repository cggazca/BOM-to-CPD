...
public class OutputResultCSV {
  ...
  private MGLogger logger = MGLogger.getLogger(OutputResultCSV.class);
  
  private String outputdir;
  
  private String fileName;
  
  private boolean suppressOK = false;
  ...
  public void setSuppressOK(boolean paramBoolean) {
    this.suppressOK = paramBoolean;
  }
  ...
  public void write(ValidateResult paramValidateResult) {
    try {
      if (this.suppressOK && paramValidateResult.getResult() == ValidateResult.Result.OK)
        return; 
      FileWriter fileWriter = new FileWriter(this.outputdir + "/" + this.outputdir, Charset.forName("UTF-8"), true);
      PrintWriter printWriter = new PrintWriter(new BufferedWriter(fileWriter));
      printWriter.write(paramValidateResult.getOutputLine() + paramValidateResult.getOutputLine());
      printWriter.close();
    } catch (IOException iOException) {
      this.logger.error(iOException.getMessage(), iOException);
    } 
...
/* Location:              C:\Users\z004ut2y\OneDrive - Siemens AG\Documents\01_Projects\Customers\Var Industries\varindustries_edm-eles-sample-dataset_2025-09-18_1349 (1)\Part Aggeration Service\ContentProviderCore.jar!\com\mentor\dms\contentprovider\cor\\utils\validate\OutputResultCSV.class
