...
@REM ******************************************************************************
@REM
@REM Script for executing Content Provider Synchronization application
set curdir=%~dp0
...
if "%_ccpid%" == "SF" (
    %JAVA_HOME%\bin\java ^
      ...
      -cp "%CLASSPATH%" ^
      com.mentor.dms.contentprovider.core.sync.ContentProviderSyncApp ^
      %*
) else (
    %JAVA_HOME%\bin\java ^
      -Xmx1024m ^
      ...
      -cp "%CLASSPATH%" ^
      com.mentor.dms.contentprovider.sync.ContentProviderSyncApp ^
      %*
)