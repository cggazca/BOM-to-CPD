...
@REM ******************************************************************************
@REM
@REM Script for executing Content Provider Validate Mapping application
set curdir=%~dp0
...
set CLASSPATH=%curdir%..\java\*;%curdir%*;%DBEDIR%\java\*;%DBEDIR%\java\DMSBrowser\plugins\*;%COMMONS_CODEC%;%COMMONS_LOGGING%;%NASHORN%
...
if "%_ccpid%" == "SF" (
    %JAVA_HOME%\bin\java ^
      ...
      -cp "%CLASSPATH%" ^
      com.mentor.dms.contentprovider.core.utils.ValidateMappingApp ^
      %*
) else (
    %JAVA_HOME%\bin\java ^
      -Xmx1024m ^
