...
@REM ******************************************************************************
@REM 
@REM Script for executing Search and Assign application
set curdir=%~dp0
...
if "%_ccpid%" == "SF" (
    %JAVA_HOME%\bin\java ^
      ...
      -cp "%CLASSPATH%" ^
      com.mentor.dms.contentprovider.core.utils.SearchAndAssignACLApp ^
      %*
) else (
    %JAVA_HOME%\bin\java ^
      -Xmx1024m ^
      ...
      -cp "%CLASSPATH%" ^
      com.mentor.dms.contentprovider.utils.SearchAndAssignACLApp ^
      %*
)